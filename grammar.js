...
let g = {
  ...
  rules: {
    document: $ => repeat(choice($._token_expanded, $.par)),

    _token_expanded: $ => choice(
      $.active_char,
      $.alignment_tab,
      $.char_ref_invalid,
      $.ignored,
    ...
    text_group: $ => group($, repeat($._token_expanded)),
    ...
    apply_group: $ => group($, $._cmd_apply, repeat($._token_expanded)),
    ...
    brack_group: $ => brackGroup($, repeat($._token_expanded)),

    // math_brack_group: $ => brackGroup($, repeat($._math_mode)),

    semi_simple_group: $ => semiSimpleGroup($, repeat($._token_expanded)),
...
function defCmd (mode, label, { cs, parameters, apply }) {
  ...
  if (mode !== '_') {
    rules.text.push($ => label === cmdSym ? $[cmdSym] : alias($[cmdSym], $[label]))
  }
}
...
function defEnv (mode, label, { name, beginParameters, endParameters, contents, bare }) {
  ...
  g.rules[envSym] = $ => seq(
    ...
    // ...(bare ? [] : [$._env_begin]),
    ...(contents ? contents($) : [$._token_expanded]),
    choice(
